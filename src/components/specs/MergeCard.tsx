    ...
    >
      ...
      {ys.map((y, i) => (
        <g key={i}>
          ...
          <circle className="merge-particle" r="2" fill="var(--color-tealhi)" opacity="0.9">
  ...
  useGSAP(() => {
    ...
    timeline
      .fromTo(
        ".merge-source",
        { autoAlpha: 0, y: 8 },
        { autoAlpha: 1, y: 0, stagger: 0.055, clearProps: "transform,opacity,visibility" },
      )
      .fromTo(
        ".merge-current-stage",
        { autoAlpha: 0, scaleX: 0.7, transformOrigin: "left center" },
        { autoAlpha: 1, scaleX: 1, clearProps: "transform,opacity,visibility" },
        "<0.04",
      )
      .fromTo(
        ".merge-destination",
        { autoAlpha: 0, x: -10, scale: 0.985 },
        { autoAlpha: 1, x: 0, scale: 1, duration: 0.3, clearProps: "transform,opacity,visibility" },
        "<0.06",
      )
      .fromTo(
        ".merge-total",
        { autoAlpha: 0, y: 5 },
        { autoAlpha: 1, y: 0, duration: 0.18, clearProps: "transform,opacity,visibility" },
        "<0.12",
      );
  ...
  return (
    <div ref={rootRef}>
      ...
      <div className="relative grid items-stretch gap-2 overflow-hidden rounded-md border border-line/50 bg-abyss/35 p-2 sm:grid-cols-[1fr_72px_1.15fr] sm:gap-0">
        ...
        <div className="relative flex flex-col justify-center gap-2">
          {platformCurrents.map((current) => (
            <div key={current.platform} className="merge-source rounded-sm border border-teal/25 bg-raise/55 px-3 py-3 shadow-[inset_-18px_0_28px_rgba(79,227,207,0.04)]">
              <div className="flex items-center justify-between gap-2">
        ...
        <div className="merge-current-stage relative min-h-6 sm:min-h-0">
          <Connector n={platformCurrents.length} />
          <p className="text-center font-mono text-[12px] text-teal sm:hidden">
            ≋ currents combine ≋
          </p>
        </div>

        <div className="merge-destination relative flex flex-col justify-center overflow-hidden rounded-sm border border-coral/40 bg-[linear-gradient(135deg,rgba(15,168,150,.09),rgba(255,133,89,.075))] px-4 py-3 shadow-[0_0_32px_rgba(79,227,207,0.09)]">
          ...
          <p className="merge-total mt-2 font-mono text-3xl leading-none font-semibold text-tealhi tabular-nums">
            {usd(combined.totalCents)}
          </p>
          <p className="mt-1.5 font-mono text-[13px] text-dim">
      ...
      <details className="group mt-2 rounded-sm border border-line/60 bg-abyss/25">
        <summary className="cursor-pointer list-none px-3 py-2 font-mono text-[13px] text-dim transition-colors hover:text-teal">
